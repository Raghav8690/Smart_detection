...
class FaceMatcher:
    ...
    def load_embeddings_from_db(self):
        try:
            print("Loading embeddings from Supabase...")
            response = supabase.table('embeddings').select('visitor_id, embeddings').execute()
            ...
            for row in response.data:
                try:
                    # Handle both 'embeddings' and 'vector' field names
                    embedding = row.get('embeddings') or row.get('vector')
                    if embedding:
                        embedding_vectors.append(embedding)
                        visitor_ids.append(row['visitor_id'])
                except Exception as e:
